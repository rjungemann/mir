...
void *_MIR_get_wrapper (MIR_context_t ctx, MIR_item_t called_func, void *hook_address) {
  ...
  /* Offsets of the immediates from the start of start_pat.  The two 5-byte
     shadow-space stores come first on win64, so every immediate sits 10 bytes
     further in than in the SysV pattern, whose prologue is 2 bytes.  */
...
void *_MIR_get_wrapper_end (MIR_context_t ctx) {
  ...
  static const uint8_t wrap_end[] = {
    ...
    0x48, 0x29, 0xc4,                   /*sub    %rax,%rsp -- aligned now */
    /* Above the 32 bytes of shadow space the callee owns, not on top of it. */
