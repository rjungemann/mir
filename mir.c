...
static void process_inlines (MIR_context_t ctx, MIR_item_t func_item) {
  ...
  for (head_func_insn = func_insn = DLIST_HEAD (MIR_insn_t, func->insns); func_insn != NULL;
    ...
    /* The callee may already have been interpreted -- normal in a REPL-style
       embedding, where earlier modules run before later ones link.  Preparing
       a function for interpretation stamps every insn's `data` with its icode
       offset, and the label bookkeeping below asserts (and needs) data == NULL.
       Drop the interpretation state; it is regenerated on the next call.

       Guarded, because within the current link batch a callee's `data` is not
       interpreter state at all: MIR_link uses it as the (void *) 1 inlining
       flag set a few lines above.  Only a callee from an earlier, already-run
       module carries a real func_desc here.  */
    if (called_func_item->data != NULL && called_func_item->data != (void *) 1)
      finish_func_interpretation (called_func_item, ctx->alloc);
